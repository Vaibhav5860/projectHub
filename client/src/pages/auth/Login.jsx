  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (msg === 'Invalid credentials') {
        setError('Invalid credentials. Please check your email/password or sign up first.')
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 px-4 transition-colors duration-200">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">Welcome back! Sign in to continue.</p>
        </div>
        ...
        <div className="bg-white/80 dark:bg-slate-800/60 backdrop-blur-lg border border-slate-200 dark:border-slate-700/50 rounded-2xl p-8 mb-10 shadow-lg dark:shadow-xl transition-colors duration-200">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <button type="button" className="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition">
                  Forgot password?
                </button>
              </div>
            ...
            >
              {isLoading ? 'Signing In...' : `Sign In${formData.role ? ` as ${formData.role.charAt(0).toUpperCase() + formData.role.slice(1)}` : ''}`}
            </button>
          </form>
          ...
          <div className="flex items-center my-6">
            ...
            <span className="px-3 text-xs text-slate-400 dark:text-slate-500 uppercase">or continue with</span>
            <div className="flex-1 border-t border-slate-200 dark:border-slate-700"></div>
          </div>
          ...
          <p className="text-center text-sm text-slate-400 dark:text-slate-500 mt-6">
            ...
            <Link to="/signup" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 font-medium transition">
              Sign up
            </Link>
          </p>
        </div>
