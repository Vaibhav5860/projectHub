...
const IntegrationSettings = () => {
  ...
  // Build state from user's saved integrations or defaults
  const [integrations, setIntegrations] = useState(() => {
    const savedIntegrations = user?.settings?.integrations || []
    return defaultIntegrations.map((def) => {
      const existing = savedIntegrations.find((s) => s.name === def.name)
      return {
        ...def,
  ...
  const toggleConnection = async (name) => {
    const updated = integrations.map((item) =>
      item.name === name
        ? { ...item, connected: !item.connected, account: !item.connected ? user?.email || '' : '' }
        : item
    )
    ...
    try {
      await updateUser({
        settings: {
          ...user?.settings,
          integrations: updated.map(({ name, connected, account }) => ({ name, connected, account })),
        },
      })
    } catch (err) {
              ...
              <div className="flex-1 min-w-0">
                ...
                {item.connected ? (
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => toggleConnection(item.name)} className="text-xs font-medium text-red-500 hover:text-red-600 dark:hover:text-red-400 transition cursor-pointer">Disconnect</button>
                  </div>
                ) : (
                  <button onClick={() => toggleConnection(item.name)} className="px-3 py-1.5 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/30 rounded-lg transition cursor-pointer">
                    Connect
                  </button>
                )}
              </div>
